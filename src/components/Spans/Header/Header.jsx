...
const Header = () => {
    ...
    //  This is needed so that inside the modal clicking the X closes
    const [showSignIn, setShowSignIn] = useState(false);    
    const [showSignUp, setShowSignUp] = useState(false);
    const flipShowSignIn = () => {
        setShowSignIn(!showSignIn);
        setShowSignUp(false);
    };
    const flipShowSignUp = () => {
    ...
    return (
        <nav className="headerContainer">
            {/* If user not null then we render header for signed in users */}
            {user !== null ? (
                <div className="headerContent">
                    ...
                    {/* On click of pfp, sign out */}
                    <ProfilePic src={'src\\assets\\testPFP.jpg'} alt="User PFP" onClick={signOut} size={40}/>
{/* TODO: deal with popup bubble on signin-out */}
                ...
                <div className="headerContent">
                    {/* Render header logo if not on main page */}
                    {location.pathname !== '/' ? <Link id="headerLogo" to='/'><h2>Doodledo</h2></Link> : null}
                    <Button onClick={flipShowSignUp} text="SIGN UP" size="wide" id="signUP" />
                    <Button onClick={flipShowSignIn} text="SIGN IN" id="signIn" />
                    <SignInUpModal show={showSignUp} changeShow={flipShowSignUp} inORup="up"/>
                    <SignInUpModal show={showSignIn} changeShow={flipShowSignIn} inORup="in"/>
                </div>
            )}
        </nav>
    );
